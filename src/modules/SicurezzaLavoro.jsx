...
export default function SicurezzaLavoro({ subTab, setSubTab }) {
  ...
  const submitAppointment = async (e) => {
    e.preventDefault();
    if (!personName.trim() || !issueDate) return;
    setApptBusy(true);
    setApptError("");
    try {
          ...
          <form onSubmit={submitAppointment} className="traccia-form">
            ...
            <div className="row-form">
              <label className="field-label">Data rilascio
                <input type="date" required value={issueDate} onChange={(e) => handleIssueChange(e.target.value)} />
              </label>
              ...
              <label className="field-label">Data scadenza
              ...
              {appointments.map((item) => {
                    ...
                    <li key={item.id} className="dish-row">
                      ...
                      <div className="nc-edit-block">
                        ...
                        <div className="row-form" style={{ marginTop: 8 }}>
                          <label className="file-drop" htmlFor={`edit-nomina-${item.id}`}>
                            <Paperclip size={15} />
                ...
                return (
                  <li key={item.id} className={"dish-row" + (info?.cls === "pill-alert" ? " row-warn" : "")}>
                    ...
                    <div className="traccia-meta">
                      <span className="doc-type-tag">Rilasciata il {fmtDate(item.issue_date)}</span>
